...
uintptr_t getRawMovesetAddress(int side);
...
int main()
{
  ...
  {
    int player = 0;
    uintptr_t rawMoveset = getRawMovesetAddress(player);
...
uintptr_t getRawMovesetAddress(int side)
{
  ...
  return game.getAddress({0x09AB2B08, 0x68, (DWORD)(0x2D0 + side * 0x10), 0x18, 0x8, 0x118, 0x0});
}
